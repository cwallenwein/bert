...
class BertModel(nn.Module):
    ...
    def forward(
        ...
        x = self.embedding(input_ids=input_ids, segment_ids=token_type_ids)
    ...
    def __init__(self, config: BertConfig):
        ...
        if config.with_next_sentence_prediction:
            self.segment_embedding_matrix = nn.Embedding(
                num_embeddings=config.n_segments, embedding_dim=config.d_model
            )
...
class BertEncoderLayer(nn.Module):
    def __init__(self, config: BertConfig):
        ...
        if config.feed_forward_activation in activations.keys():
            self.feed_forward = nn.Sequential(
                nn.Linear(
                    config.d_model,
                    config.feed_forward_intermediate_size,
            ...
            self.feed_forward = nn.Sequential(
                ...
                GatedLinearUnit(),
                nn.Linear(
                    config.feed_forward_intermediate_size // 2,
                    config.d_model,
                    bias=config.feed_forward_bias,
                ),
            )
        ...
        self.layer_norm1 = nn.LayerNorm(normalized_shape=config.d_model, eps=1e-6)
        self.layer_norm2 = nn.LayerNorm(normalized_shape=config.d_model, eps=1e-6)
    ...
    def forward(
        self,
        x: Annotated[Tensor, torch.int16, "batch seq_len d_model"],
        attention_mask: Annotated[Tensor, torch.int16, "batch seq_len"],
    ):
        ...
        x = x + self.feed_forward_dropout(self.feed_forward(x))
